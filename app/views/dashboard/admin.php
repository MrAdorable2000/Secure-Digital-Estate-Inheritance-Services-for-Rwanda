...
$officers_count = (int)($officers_count ?? 0); 
...
.dashboard-admin .stat-card { transition: box-shadow var(--duration-base), transform var(--duration-base); }
.dashboard-admin .card:hover,
.dashboard-admin .stat-card:hover { box-shadow: var(--shadow-md); transform: translateY(-1px); }
</style>
...
<div class="dashboard-welcome" style="padding:var(--sp-8) 0 var(--sp-6);margin-bottom:var(--sp-8);">
    ...
    <p style="font-size:var(--text-base);color:var(--color-text-secondary);margin:0 0 var(--sp-3) 0;line-height:1.6;">Here is your administration overview.</p>
    <span class="dashboard-welcome__role">Administrator</span>
</div>
...
<div class="dashboard-grid">
    ...
    <div class="stat-card green">
        ...
        <div class="stat-card-info">
            <span class="stat-card-value"><?= $officers_count ?></span>
            <span class="stat-card-label">Officers</span>
        </div>
    </div>
    ...
    <div class="stat-card amber">
        ...
        <div class="stat-card-info">
            <span class="stat-card-value"><?= is_countable($departments) ? count($departments) : 0 ?></span>
        ...
        <div class="card-body" style="padding:0;">
            <?php if (empty($recent_activities)): ?>
                <div class="empty-state" style="padding:var(--sp-12) var(--sp-6);">
                    ...
                    <p class="text-muted" style="font-size:var(--text-sm);margin:0 0 var(--sp-1) 0;font-weight:500;">No activity data available yet</p>
                    <p style="font-size:var(--text-xs);color:var(--color-text-light);margin:0;">Activity will appear here once users start interacting with the system</p>
                </div>
            <?php else: ?>
                <div class="table-container" style="border:none;border-radius:0;">
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td style="white-space:nowrap;"><?= e(format_datetime($activity['created_at'] ?? '')) ?></td>
                                <td><?= e($activity['user_name'] ?? 'System') ?></td>
                                <td><span class="badge badge-info"><?= e($activity['action'] ?? 'N/A') ?></span></td>
                                <td><?= e($activity['module'] ?? 'N/A') ?></td>
    ...
    <div class="card">
        ...
        <div class="card-body" style="padding:0;">
            <?php if (empty($departments)): ?>
                <div class="empty-state" style="padding:var(--sp-12) var(--sp-6);">
                    <div style="width:56px;height:56px;border-radius:var(--radius-xl);background:var(--bg-subtle);display:inline-flex;align-items:center;justify-content:center;margin-bottom:var(--sp-5);color:var(--color-text-light);opacity:0.5;">
                        <svg viewBox="0 0 24 24" width="28" height="28" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
                    </div>
                    <p class="text-muted" style="font-size:var(--text-sm);margin:0 0 var(--sp-1) 0;font-weight:500;">No departments configured yet</p>
                    <p style="font-size:var(--text-xs);color:var(--color-text-light);margin:0;">Departments can be created from the administration settings</p>
                </div>
            <?php else: ?>
                <div class="table-container" style="border:none;border-radius:0;">
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><?= e($dept['name'] ?? 'N/A') ?></td>
                                <td>
                                    <?php $deptStatus = strtolower($dept['status'] ?? 'active'); ?>
                                    <?php if ($deptStatus === 'active'): ?>
                                        <span class="badge badge-success">Active</span>
                                    <?php elseif ($deptStatus === 'inactive'): ?>
                                        <span class="badge badge-warning">Inactive</span>
                                    <?php else: ?>
                                        <span class="badge badge-info"><?= e(ucfirst($deptStatus)) ?></span>
                                    <?php endif; ?>
                                </td>
                            </tr>
                            <?php endforeach; ?>
