<?php
/**
 * R-DEIP 419 Error Page — COMPLETELY SELF-CONTAINED
 */
$code = 419;
$title = 'Page Expired';
$message = $message ?? 'This page has expired due to inactivity. Please refresh and try again.';
...
:root{--c-primary:#00693E;--c-primary-light:#00A651;--c-accent:#E5A100;--c-cyan:#0891B2;--c-blue:#1B3A6B;--bg:#0B1120;--surface:rgba(255,255,255,0.04);--border:rgba(255,255,255,0.08);--text:#F1F5F9;--muted:#94A3B8;--dim:#64748B;--font:'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif}
...
.bg::before{content:'';position:absolute;top:-50%;left:-50%;width:200%;height:200%;background:radial-gradient(ellipse 600px 600px at 35% 30%,rgba(8,145,178,0.3),transparent),radial-gradient(ellipse 500px 500px at 65% 70%,rgba(0,166,81,0.25),transparent),radial-gradient(ellipse 400px 400px at 50% 50%,rgba(27,58,107,0.2),transparent);animation:drift 20s ease-in-out infinite alternate}
...
.orb-1{width:300px;height:300px;top:-80px;right:-60px;background:var(--c-cyan);animation-delay:0s}
.orb-2{width:200px;height:200px;bottom:-40px;left:-40px;background:var(--c-primary-light);animation-delay:-5s;animation-duration:18s}
...
.icon-ring{position:absolute;inset:0;border-radius:50%;border:2px solid transparent;border-top-color:var(--c-cyan);border-right-color:rgba(8,145,178,0.3);animation:spin 8s linear infinite}
.icon-ring-inner{position:absolute;inset:12px;border-radius:50%;border:1.5px solid transparent;border-bottom-color:var(--c-cyan);border-left-color:rgba(8,145,178,0.15);animation:spin 12s linear infinite reverse}
@keyframes spin{to{transform:rotate(360deg)}}
.icon-bg{position:absolute;inset:20px;border-radius:50%;background:var(--c-cyan);opacity:.08;box-shadow:0 0 60px rgba(8,145,178,0.25)}
.icon-svg{position:relative;z-index:2;width:56px;height:56px}
.error-code{font-size:7rem;font-weight:800;line-height:1;letter-spacing:-.04em;margin-bottom:.25rem;background-image:linear-gradient(135deg,#0891B2,#22D3EE,#67E8F9,#22D3EE,#0891B2);background-size:200% 100%;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:shimmer 3s ease-in-out infinite}
...
<div class="content">
    <div class="icon-wrap">
        ...
        <svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="#22D3EE" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
    </div>
    ...
    <div class="actions">
        <a href="javascript:location.reload()" class="btn btn-primary">Refresh Page<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2v4h-4M2 14v-4h4"/><path d="M14 6A6 6 0 0 0 4.5 3.5L2 6M2 10a6 6 0 0 0 9.5 2.5L14 10"/></svg></a>
