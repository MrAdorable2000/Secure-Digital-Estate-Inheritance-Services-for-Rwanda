<?php
/**
 * R-DEIP 429 Error Page — COMPLETELY SELF-CONTAINED
 */
$code = 429;
$title = 'Too Many Requests';
$message = $message ?? 'You have made too many requests in a short period. Please wait a moment and try again.';
...
:root{--c-primary:#00693E;--c-primary-light:#00A651;--c-accent:#E5A100;--c-blue:#1B3A6B;--c-blue-light:#3B82F6;--bg:#0B1120;--surface:rgba(255,255,255,0.04);--border:rgba(255,255,255,0.08);--text:#F1F5F9;--muted:#94A3B8;--dim:#64748B;--font:'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif}
...
.bg::before{content:'';position:absolute;top:-50%;left:-50%;width:200%;height:200%;background:radial-gradient(ellipse 600px 600px at 20% 40%,rgba(59,130,246,0.3),transparent),radial-gradient(ellipse 500px 500px at 80% 60%,rgba(27,58,107,0.25),transparent),radial-gradient(ellipse 400px 400px at 50% 50%,rgba(0,166,81,0.2),transparent);animation:drift 20s ease-in-out infinite alternate}
...
.orb-1{width:300px;height:300px;top:-80px;right:-60px;background:var(--c-blue-light);animation-delay:0s}
...
.icon-ring{position:absolute;inset:0;border-radius:50%;border:2px solid transparent;border-top-color:var(--c-blue-light);border-right-color:rgba(59,130,246,0.3);animation:spin 8s linear infinite}
.icon-ring-inner{position:absolute;inset:12px;border-radius:50%;border:1.5px solid transparent;border-bottom-color:var(--c-blue-light);border-left-color:rgba(59,130,246,0.15);animation:spin 12s linear infinite reverse}
@keyframes spin{to{transform:rotate(360deg)}}
.icon-bg{position:absolute;inset:20px;border-radius:50%;background:var(--c-blue-light);opacity:.08;box-shadow:0 0 60px rgba(59,130,246,0.25)}
.icon-svg{position:relative;z-index:2;width:56px;height:56px}
.error-code{font-size:7rem;font-weight:800;line-height:1;letter-spacing:-.04em;margin-bottom:.25rem;background-image:linear-gradient(135deg,#3B82F6,#60A5FA,#93C5FD,#60A5FA,#3B82F6);background-size:200% 100%;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:shimmer 3s ease-in-out infinite}
...
.countdown{margin-top:2rem;animation:up .8s .45s cubic-bezier(.16,1,.3,1) both}
.countdown-text{font-size:.9rem;color:var(--dim);margin-bottom:.5rem}
.countdown-bar{width:200px;height:4px;background:var(--surface);border-radius:2px;margin:0 auto;overflow:hidden}
.countdown-fill{height:100%;background:linear-gradient(90deg,var(--c-blue-light),#60A5FA);border-radius:2px;animation:fill-bar 60s linear forwards}
@keyframes fill-bar{from{width:100%}to{width:0%}}
...
@media(prefers-reduced-motion:reduce){.bg::before,.orb,.particle,.icon-wrap,.icon-ring,.icon-ring-inner,.error-code,.content,.error-title,.error-desc,.actions,.countdown,footer{animation:none!important}.content,.error-title,.error-desc,.actions,.countdown,footer{opacity:1!important;transform:none!important;filter:none!important}.countdown-fill{animation:none!important;width:100%!important}}
...
<div class="content">
    <div class="icon-wrap">
        ...
        <svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="#60A5FA" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
    </div>
    ...
    <div class="actions">
        <a href="javascript:location.reload()" class="btn btn-primary">Try Again<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2v4h-4M2 14v-4h4"/><path d="M14 6A6 6 0 0 0 4.5 3.5L2 6M2 10a6 6 0 0 0 9.5 2.5L14 10"/></svg></a>
        <a href="<?php echo $esc($base); ?>/" class="btn btn-ghost">Go to Homepage</a>
    </div>
    <div class="countdown">
        <div class="countdown-text">Please wait before trying again</div>
