<?php
/**
 * R-DEIP 403 Error Page — COMPLETELY SELF-CONTAINED
 */
$code = 403;
$title = 'Access Denied';
$message = $message ?? 'You do not have permission to access this resource. If you believe this is an error, contact your administrator.';
...
$btnLabel = $isLoggedIn ? 'Go to Dashboard' : 'Sign In';
$btnHref = $isLoggedIn ? ($base . '/dashboard') : ($base . '/login');
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
:root{--c-primary:#00693E;--c-primary-light:#00A651;--c-accent:#E5A100;--c-warning:#F59E0B;--c-blue:#1B3A6B;--bg:#0B1120;--surface:rgba(255,255,255,0.04);--border:rgba(255,255,255,0.08);--text:#F1F5F9;--muted:#94A3B8;--dim:#64748B;--font:'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;--mono:'SF Mono','Fira Code',monospace}
...
.bg::before{content:'';position:absolute;top:-50%;left:-50%;width:200%;height:200%;background:radial-gradient(ellipse 600px 600px at 30% 25%,rgba(245,158,11,0.3),transparent),radial-gradient(ellipse 500px 500px at 70% 75%,rgba(229,161,0,0.25),transparent),radial-gradient(ellipse 400px 400px at 50% 50%,rgba(27,58,107,0.2),transparent);animation:drift 20s ease-in-out infinite alternate}
...
.orb-1{width:300px;height:300px;top:-80px;right:-60px;background:var(--c-warning);animation-delay:0s}
...
.icon-ring{position:absolute;inset:0;border-radius:50%;border:2px solid transparent;border-top-color:var(--c-warning);border-right-color:rgba(245,158,11,0.3);animation:spin 8s linear infinite}
.icon-ring-inner{position:absolute;inset:12px;border-radius:50%;border:1.5px solid transparent;border-bottom-color:var(--c-warning);border-left-color:rgba(245,158,11,0.15);animation:spin 12s linear infinite reverse}
@keyframes spin{to{transform:rotate(360deg)}}
.icon-bg{position:absolute;inset:20px;border-radius:50%;background:var(--c-warning);opacity:.08;box-shadow:0 0 60px rgba(245,158,11,0.25)}
.icon-svg{position:relative;z-index:2;width:56px;height:56px}
.error-code{font-size:7rem;font-weight:800;line-height:1;letter-spacing:-.04em;margin-bottom:.25rem;background-image:linear-gradient(135deg,#F59E0B,#FBBF24,#FDE68A,#FBBF24,#F59E0B);background-size:200% 100%;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:shimmer 3s ease-in-out infinite}
...
@media(prefers-reduced-motion:reduce){.bg::before,.orb,.particle,.icon-wrap,.icon-ring,.icon-ring-inner,.error-code,.content,.error-title,.error-desc,.actions,footer{animation:none!important}.content,.error-title,.error-desc,.actions,footer{opacity:1!important;transform:none!important;filter:none!important}}
...
<div class="content">
    <div class="icon-wrap">
        ...
        <svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="#FBBF24" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
    </div>
    ...
    <div class="actions">
        <a href="<?php echo $esc($btnHref); ?>" class="btn btn-primary"><?php echo $esc($btnLabel); ?><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 8h10m0 0L9 4m4 4L9 12"/></svg></a>
